...
| **Startup check** | While the game loads, checks that every enabled fix is in place, that Harmony-patched frames resolve (so reports name the right mod), and that the Faster Game Loading build, its settings and the versions are the tested ones. Shown in the main-menu corner the way the Harmony mod shows its version, and at the top of the settings page and the diagnostic report. |
| **Repeated-error finder** | Names the mod whose code keeps throwing the same error, by reading the exception object in Unity's exception formatter. That works even when the Harmony mod's trace cache leaves only "Duplicate stacktrace" in the log. Notice at 100 repeats, on screen once at 1,000. |
